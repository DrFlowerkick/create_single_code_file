...
impl<O, S> CgData<O, S> {
    ...
    pub fn get_path_target(
        ...
        let segments = match path.extract_path() {
            SourcePath::Name(segments) => segments,
            SourcePath::Rename(segments, renamed_id) => {
            ...
            SourcePath::Group => return Ok(PathTarget::Group),
        };
                ...
                _ => {
                        ...
                        .find(|(_, m, _)| segment == m)
                    {
                        ...
                        match self.get_path_target(use_module_index, use_tree)? {
                            PathTarget::ExternalPackage => return Ok(PathTarget::ExternalPackage),
                            PathTarget::Glob(_) | PathTarget::Group => {
                                unreachable!(
                            ...
                            PathTarget::Item(item_index) => {
                                ...
                                let name = self
                                    .get_name_of_crate_or_module(item_index)
        ...
        if let Some(renamed_id) = rename {
            return Ok(PathTarget::ItemRenamed(current_index, renamed_id));
        }
